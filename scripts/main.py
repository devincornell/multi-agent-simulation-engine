...
class HexGridViz:
    '''Manages translations between hex space and pixel space.'''
    ...
    ################################ Unit Conversion ################################
    def coord_to_px(self, coord: mase.HexCoord, flat_top: bool = True) -> tuple[mase.XPixelCoord, mase.YPixelCoord]:
        '''Convert hex coordinate to pixel coordinate.'''
        cart_coord = coord.to_cartesian()
        return (
            round((cart_coord.x - self.offset[0]) * self.scale[0]),
            round((cart_coord.y - self.offset[1]) * self.scale[1]),
        )
    
    #def px_to_coord(self, px: tuple[mase.XPixelCoord, mase.YPixelCoord]) -> mase.HexCoord:
    #    '''Convert pixel coordinate to hex coordinate.'''
    #    x, y = (
    #        (px[0] / self.scale[0]) + self.offset[0],
    #        (px[1] / self.scale[1]) + self.offset[1],
    #    )
    #    return mase.HexCoord.from(x, y)
...
def main():
    ...
    viz = HexGridViz.from_points((800, 600), region)
    ...
    with mase.PyGameCtx(size=viz.screen_size, title='Hexagonal Grid Game') as ctx:
        ...
        bg_image = ctx.load_image('../data/hex_bg/rock_hexagonal_noborder.png', size=(20,20))
        ...
        for i, events in ctx.display_iter(frame_limit=5):
            ...
            for pos in region:
                ctx.blit_image(bg_image, viz.coord_to_px(pos))
